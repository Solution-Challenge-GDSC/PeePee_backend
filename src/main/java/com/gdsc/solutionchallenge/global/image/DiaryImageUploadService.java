...
public class DiaryImageUploadService {
    ...
    public void deleteImage(String filename) {
        ...
        DiaryPhoto diaryPhoto = diaryPhotoRepository.findByFileName(filename).orElseThrow(() -> {
            throw new ApiException(ApiResponseStatus.BAD_REQUEST);
        });

        Blob blob = storage.get(bucketName, diaryPhoto.getFileName());
        Storage.BlobSourceOption precondition = Storage.BlobSourceOption.generationMatch(blob.getGeneration());
        storage.delete(bucketName, diaryPhoto.getFileName(), precondition);
    }
}